export default {
  ...
  async fetchDomaineData({ commit }) {
    await this.$axios
      .$get("api/mysys/domaines")
      .then(res => {
        commit("SET_DOMAINES", res);
  ...
  async fetchThemeData({ commit }) {
    await this.$axios
      .$get("api/mysys/themes")
      .then(res => {
        commit("SET_THEMES", res);
  ...
  async fetchFormationData({ commit }) {
    await this.$axios
      .$get("api/mysys/formations")
      .then(res => {
        commit("SET_FORMATIONS", res);
  ...
  //   await dispatch("fetchDomaineData");
